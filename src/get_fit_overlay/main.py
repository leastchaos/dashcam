...
if __name__ == "__main__":
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler(Path.cwd() / "logs/garmin_export.log")],
    )
    os.makedirs(FIT_FOLDER, exist_ok=True)
    os.makedirs(OVERLAY_FOLDER, exist_ok=True)
    latest_fit_file = download_latest_activity(output_folder=FIT_FOLDER)
    output_overlay_file = OVERLAY_FOLDER / latest_fit_file.with_suffix(".mp4")
    generate_dashboard(fit=latest_fit_file, output=output_overlay_file)
